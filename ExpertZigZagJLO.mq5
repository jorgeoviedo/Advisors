...
void OnTradeTransaction(const MqlTradeTransaction &_trans, const MqlTradeRequest &_req, const MqlTradeResult &_res) {
	if (((ENUM_TRADE_REQUEST_ACTIONS)_req.action == TRADE_ACTION_DEAL) && (_res.retcode == TRADE_RETCODE_DONE) &&
		((ENUM_TRADE_TRANSACTION_TYPE)_trans.type == TRADE_TRANSACTION_REQUEST)) {
		order.orderPaint();
	}
}
//+-------------------------------------------------------------------------------------------------------------------+
